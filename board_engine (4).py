...
for (first,last,state),rws in grouped.items():
    ...
    for r in rws:
        ...
        if w: wt_votes[w]+=1
    if is_girl: continue                                # BOYS-ONLY pass: girls board untouched
...
for row in board:
    ...
    for a in row['athletes']:
        ...
        out=dict(a)
        if board_is_girls and not FULL:
            pass                                        # BOYS-ONLY pass: girls frozen verbatim
        elif key in ANCHORS:
            anchor_frozen+=1
...
print("\n"+"="*72)
print(f"BOARD ENGINE — {'FULL REBUILD' if FULL else 'IL-ONLY' if ILONLY else 'NJ-ONLY' if NJONLY else 'SCOPED RECOMPUTE'} — {'APPLYING' if APPLY else 'PREVIEW (no writes)'}")
print("="*72)
