...
T1 = ['CA','OH','NJ','PA','IA']  # was: PA OH IL NJ MN IA OK CA NE WI — IL/MN/OK/NE/WI demoted to T2 (2026-06)
T2 = ['NY','MI','MO','CO','VA','MD','IN','TX','NC','KS','UT','GA','IL','MN','OK','NE','WI']
EVENT_MULT = {
...
def calc_wpom(state, natl_results, state_titles=0, nwi_opponents=None):
    ...
    for r in natl_results:
        ...
        em = EVENT_MULT.get(r['event'], 0.80)
        events.append({
    ...
    title_per = 0.15 if tier == 1 else 0.12 if tier == 2 else 0.08
...
def main():
    ...
    for (first, last, state), wrestler_rows in grouped.items():
        ...
        result = calc_wpom(state, natl_results)
        score = result['score']
