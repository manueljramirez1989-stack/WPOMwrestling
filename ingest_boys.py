...
def replace_rows(table, uuid_field, uuid, rows):
    ...
    # merge-duplicates makes re-runs and any stray rows self-heal instead of 409
    hh = {**H, "Prefer": "resolution=merge-duplicates"}
    for i in range(0, len(rows), 500):
        requests.post(f"{URL}/rest/v1/{table}", headers=hh, json=rows[i:i+500], timeout=120).raise_for_status()
...
def main():
    ...
    # dedup on (event_uuid, wrestler_guid): a name-collision (two different kids,
    # same name) would violate the unique constraint. Keep the BEST placement.
    best = {}
    for r in gr:
        k = (r["event_uuid"], r["wrestler_guid"])
        if k not in best or r["placement"] < best[k]["placement"]:
            best[k] = r
    if len(best) < len(gr):
        print(f"  dedup: {len(gr)-len(best)} same-name/same-event collision(s) "
              f"resolved (kept best placement)")
    gr = list(best.values())
