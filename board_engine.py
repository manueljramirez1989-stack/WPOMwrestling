...
ILONLY = "--il-only" in sys.argv     # scope this pass to IL only (same engine, isolated review)
NJONLY = "--nj-only" in sys.argv     # scope this pass to NJ only
GIRLS  = "--girls" in sys.argv       # GIRLS-ONLY recalibration: score girls via is_girl ladder, freeze boys
...
AFFECTED  = {'IL'} if ILONLY else ({'NJ'} if NJONLY else (DEMOTED | EVT_BUMP))
...
for (first,last,state),rws in grouped.items():
    st=(state or '').upper()
    if not GIRLS and not FULL and st not in AFFECTED: continue   # SCOPED: only changed states (girls pass ignores state scope)
    # IL now flows through here too: IL kids WITH flo_results rows (national
    # results — Rocco's Walsh+S32, Mastny's Fargo+S32) recompute normally at
    # Tier-2. IL state-title-only kids (no rows) are frozen in the merge.
    ...
    for r in rws:
        ...
        if w: wt_votes[w]+=1
    if GIRLS:
        if not is_girl: continue                        # GIRLS-ONLY pass: skip boys
    else:
        if is_girl: continue                            # boys pass: skip girls
    if not natl or not wt_votes: continue
    rec=SA.calc_wpom(state, natl, is_girl=is_girl)        # girls use national-first ladder
...
print("[6/6] merging (anchors frozen, IL via flo_results, NIL reassigned)...")
changes=[]; anchor_frozen=0; il_frozen=0; il_recomputed=0; recomputed=0; unexpected=[]
new_board={(row['board'],row['weight']):[] for row in board}

for row in board:
    board_is_girls = (row['board']=='girls')
    for a in row['athletes']:
        nm=(a.get('name') or '').strip().lower(); st=(a.get('state') or '').strip().upper()
        key=(nm,st); old=float(a.get('score') or 0); nwi=float(a.get('nwi') or 0)
        out=dict(a)
        if GIRLS:
            if board_is_girls and key in scored:
                s=scored[key]; addnwi=min(round(nwi_raw.get(key,0),2),0.40)
                new=round(min(4.4, s['base']+addnwi),2)   # GIRLS computed cap 4.4 — 4.5+ is anchor-only (multi S32+Fargo titles AND Senior team)
                if abs(new-old)>1e-9:
                    out['score']=new; out['nwi']=addnwi; out['events']=s['events']
                    if out.get('verdict') in STD_VERDICTS: out['verdict']=SA.verdict_for_score(new)
                    out['nil']=''; recomputed+=1            # girls: no $ NIL bands (tier labels only)
                    changes.append((a.get('name'),st,old,new,'girls-recompute'))
            # boys (and unscored girls) frozen verbatim in girls pass
        elif board_is_girls and not FULL:
            pass                                        # boys pass: girls frozen verbatim
        elif key in ANCHORS:
            anchor_frozen+=1
        elif key in scored and (FULL or st in AFFECTED):
            # recompute from flo_results — CA/MN/OK/NE/WI AND IL-with-national (Rocco).
            # Idempotent: always derived from raw placements, never from current score.
            s=scored[key]; addnwi=min(round(nwi_raw.get(key,0),2),0.40)
        ...
        elif st=='IL' and not FULL and key not in scored:
            # IL state-title-only: no flo_results rows. Already at correct x0.85 from
            # the prior pass — FROZEN (not re-scaled, so a re-run can't double-penalize).
            # Made first-class later via the IHSA article re-parse into flo_results.
            il_frozen+=1
...
print("\n"+"="*72)
print(f"BOARD ENGINE — {'GIRLS-ONLY' if GIRLS else 'FULL REBUILD' if FULL else 'IL-ONLY' if ILONLY else 'NJ-ONLY' if NJONLY else 'SCOPED RECOMPUTE'} — {'APPLYING' if APPLY else 'PREVIEW (no writes)'}")
print("="*72)
print(f"  anchors frozen:            {anchor_frozen}")
print(f"  recomputed from flo_results: {recomputed}   (of which IL-with-national: {il_recomputed})")
print(f"  IL state-only frozen (pending IHSA backfill): {il_frozen}")
